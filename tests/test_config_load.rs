...
fn test_load_drivers_toml() {
    ...
    assert!(duckdb.version_req.is_some(), "DuckDB should have version requirement");
    assert!(duckdb.artifact_filter.is_some(), "DuckDB should have artifact filter");
    ...
    assert!(version_req.matches(&semver::Version::parse("0.8.0").unwrap()), "Should match 0.8.0");
    assert!(version_req.matches(&semver::Version::parse("1.4.0").unwrap()), "Should match 1.4.0");
    assert!(!version_req.matches(&semver::Version::parse("0.7.1").unwrap()), "Should not match 0.7.1");
    ...
    assert!(duckdb.matches_artifact("libduckdb-osx-universal.zip"), "Should match libduckdb");
    assert!(duckdb.matches_artifact("libduckdb-linux-amd64.zip"), "Should match libduckdb");
    assert!(!duckdb.matches_artifact("duckdb_cli-osx-universal.zip"), "Should not match CLI");
    assert!(!duckdb.matches_artifact("duckdb_jdbc.jar"), "Should not match JDBC");
